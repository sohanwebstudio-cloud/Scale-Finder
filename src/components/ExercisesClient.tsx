    ...
    <div className="flex flex-col items-center">
      <svg width={SVG_W} height={SVG_H} className="overflow-visible" aria-label={`Diagramme ${chord.name}`}>
        ...
        {chord.frets.map((fret, si) => {
          ...
          if (fret === 'x') {
            return <text key={si} x={x} y={NUT_Y - 9} textAnchor="middle" fill="#8a8678" fontSize={12} fontWeight="700">×</text>
          }
          if (fret === 0) {
          ...
          if (isBarre) {
            return <text key={si} x={x} y={cy + 4} textAnchor="middle" fill="#141414" fontSize={9} fontWeight="800">{finger}</text>
          }
          const color = finger ? FINGER_COLORS[finger]?.text ?? '#c9c4b4' : '#c9c4b4'
          return (
            <g key={si}>
              <circle cx={x} cy={cy} r={DR} fill={color} opacity={0.9} />
              {finger > 0 && (
                <text x={x} y={cy + 3.5} textAnchor="middle" fill="#141414" fontSize={9} fontWeight="800">{finger}</text>
              )}
            </g>
          )
        })}
        ...
        {tabStrings.map((tabStr, si) => {
          ...
          return (
            <div key={si} className="flex items-center">
              <span className="mr-2 w-4 shrink-0 text-[11px] text-neutral-500">{STRING_NAMES[si]}</span>
              ...
              {notes.map((note, ni) => {
                ...
                return (
                  <span key={ni} className="flex items-center">
                    {isH && <span className="text-xs text-neutral-400">h</span>}
                    {isP && <span className="text-xs text-neutral-400">p</span>}
                    <span style={{ color }} className="font-bold">{isH || isP ? note.slice(1) : note}</span>
      ...
      {fingers.map((f, i) => (
        <span key={i} className="flex items-center gap-1">
          <span
            ...
            style={{ background: FINGER_COLORS[f]?.bg, color: FINGER_COLORS[f]?.text, border: `1px solid ${FINGER_COLORS[f]?.text}` }}
          >
            {f}
          </span>
          {i < fingers.length - 1 && <span className="text-xs text-neutral-400">–</span>}
        </span>
      ))}
    </div>
  )
  ...
  return (
    ...
    >
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <span className="border border-neutral-300 px-2 py-0.5 text-[10px] uppercase tracking-[0.15em] text-neutral-500">
          {ex.category}
        </span>
      </div>
      <h3 className="mb-0.5 text-base font-medium text-ink">{ex.title}</h3>
      <p className="mb-2 text-[11px] uppercase tracking-[0.18em] text-neutral-500">{ex.subtitle}</p>
      <p className="mb-4 text-sm leading-relaxed text-neutral-600">{ex.description}</p>
      <button
        onClick={() => onSetBpm(ex.tempo)}
        className="mb-4 flex items-center gap-3 border border-ink bg-paper px-3 py-1.5 text-sm transition-colors hover:bg-cream"
      >
        <span className="font-mono font-bold">{ex.tempo}</span>
        <span className="text-[11px] uppercase tracking-wider text-neutral-500">BPM</span>
        <span className="text-xs text-riso-pink-deep">↓ métronome</span>
      </button>
      ...
      <div className="mt-4 bg-cream px-4 py-3">
        <p className="mb-1 text-[11px] uppercase tracking-[0.18em] text-neutral-500">Conseil</p>
        <p className="text-sm leading-relaxed text-neutral-600">{ex.tip}</p>
      </div>
    </article>
  )
}
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap justify-center gap-6 sm:justify-start">
        {ex.chords.map((chord, i) => <ChordDiagram key={i} chord={chord} />)}
      </div>
      <div className="mt-4 flex items-center gap-1.5 font-mono text-sm text-neutral-400">
        {ex.chords.map((chord, i) => (
          <span key={i} className="flex items-center gap-1.5">
            <span>/ / / /</span>
            {i < ex.chords.length - 1 && <span className="mx-1">|</span>}
          </span>
        ))}
      </div>
      <div className="mt-4 bg-cream px-4 py-3">
...
function Metronome({ bpm, onBpmChange }: { bpm: number; onBpmChange: (v: number) => void }) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-ink bg-paper">
      <div className="mx-auto flex max-w-4xl items-center gap-5 px-4 py-3 sm:px-6">
        ...
        <div className="flex min-w-0 flex-1 items-center gap-3">
          <button
            ...
            className="flex h-7 w-7 shrink-0 items-center justify-center border border-ink transition-colors hover:bg-cream"
          >−</button>
          <input
        ...
        <button
          ...
          className={`shrink-0 border px-5 py-2 text-sm font-medium transition-colors ${
            isPlaying
              ...
              : 'border-ink bg-ink text-paper hover:border-riso-red hover:bg-riso-red'
          }`}
        >
    ...
    <>
      ...
      <section className="border border-ink">
        <header className="flex flex-wrap items-baseline justify-between gap-2 border-b border-ink px-4 py-3 sm:px-6">
          <h2 className="text-sm font-medium uppercase tracking-[0.18em]">Doigts</h2>
        </header>
        <div className="flex flex-wrap items-center gap-5 px-4 py-4 sm:px-6">
          {[1, 2, 3, 4].map((f) => (
